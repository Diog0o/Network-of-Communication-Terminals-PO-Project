...
abstract public class Terminal implements Serializable, Comparable<Terminal> /* FIXME maybe addd more interfaces */{
	...
	public void set_ongoingcommunication(Communication ongoingcommunication) {
	...
	public long ShowTerminalPayments() {
		long res =0;
	...
	public boolean canEndCurrentCommunication() {
		if (_state.ShowTerminalStat().equals("BUSY") && _ongoingcommunication != null){
			return true;
		}
		return false;
  }
	...
	public boolean canStartCommunication() {
		if (_state.ShowTerminalStat().equals("OFF") || _state.ShowTerminalStat().equals("BUSY")){
			return false;
		}
		return true;
 }
	...
	public void updatePendingNotifications(){
		if (_previousStat.ShowTerminalStat().equals("OFF") && _state.ShowTerminalStat().equals("SILENCE")){
			for (int i = 0; i < _pendingnotifications.size(); i++){
				O2S notification = new O2S(_pendingnotifications.get(i).get_departureterminal(),_pendingnotifications.get(i).get_arrivalterminal());
				_pendingnotifications.set(i, notification);
			}
		}
		if (_previousStat.ShowTerminalStat().equals("OFF") && _state.ShowTerminalStat().equals("IDLE")){
			for (int i = 0; i < _pendingnotifications.size(); i++){
				O2I notification = new O2I(_pendingnotifications.get(i).get_departureterminal(),_pendingnotifications.get(i).get_arrivalterminal());
				_pendingnotifications.set(i, notification);
			}
		}
		if (_previousStat.ShowTerminalStat().equals("BUSY") && _state.ShowTerminalStat().equals("IDLE")){
			for (int i = 0; i < _pendingnotifications.size(); i++){
				B2I notification = new B2I(_pendingnotifications.get(i).get_departureterminal(),_pendingnotifications.get(i).get_arrivalterminal());
				_pendingnotifications.set(i, notification);
			}
		}
		if (_previousStat.ShowTerminalStat().equals("SILENCE") && _state.ShowTerminalStat().equals("IDLE")){
			for (int i = 0; i < _pendingnotifications.size(); i++){
				S2I notification = new S2I(_pendingnotifications.get(i).get_departureterminal(),_pendingnotifications.get(i).get_arrivalterminal());
				_pendingnotifications.set(i, notification);
			}
		}
	}
	public void sendpendingnotification() {
		for (int i = 0; i < _pendingnotifications.size(); i++) {
			...
			boolean check = false;
			for (int j = 0; j < n.get_departureterminal().get_owner().get_notifications().size(); j++) {
				if (n.get_arrivalterminal() == n.get_departureterminal().get_owner().get_notifications().get(j).get_arrivalterminal()) {
					check = true;
				}
			}
			if (check == false) {
	...
	public int compareTo(Terminal other){
		return _key.compareTo(other.get_key());
	}
}
