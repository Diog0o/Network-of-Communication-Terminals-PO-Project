...
//CONSOLA DE UM TERMINAL-----------------------------------------------------------------------------------------------
		...
		if (terminalKey.length() == 6 && terminalKey.matches("(\\d{6})")) {
			...
			if (t != null) {
				if (!t.get_state().ShowTerminalStat().equals("IDLE")) {
					if (!t.get_state().ShowTerminalStat().equals("BUSY")) {
		...
		if (terminalKey.length() == 6 && terminalKey.matches("(\\d{6})")) {
			...
			if (t != null) {
				if (terminalKeytoadd.length() == 6 && terminalKeytoadd.matches("(\\d{6})")) {
					...
					if (t2 != null) {
						if (!t.get_friendlist().containsKey(terminalKeytoadd) && !t.get_key().equals(t2.get_key())) {
					...
					if (t2 != null) {
						if (t.get_state().startTextCommunication()) {
							if (t2.get_state().receiveTextCommunication()) {
								Text c = new Text(_communicationid++, 0, t, t2, text);
								c.set_cost(t.get_owner().get_level().cost(c));
								_communications.put(c.get_id(), c);
								...
								if (t.get_owner().isNotifications()) {
									...
									DefaultState defalt = new DefaultState(t,t2);
	...
	public void StartInteractiveCommunication (String terminalKeysender, String terminalKeyreceiver, String videoorvoice) throws UnknownTerminalException, TerminalKeyInvalidException, CantStartCommunicationException, UnsupportedAtOriginException, DestinationisOffException, DestinationIsBusyException, DestinationIsSilentException, UnsupportedAtDestinationException {
		Terminal t = _terminals.get(terminalKeysender);
		Terminal t2 = _terminals.get(terminalKeyreceiver);
		if (t != null){
			if (terminalKeysender.length() == 6 && terminalKeysender.matches("(\\d{6})")){
				if (t2!= null){
					if(terminalKeyreceiver.length() == 6 && terminalKeyreceiver.matches("(\\d{6})")){
						if(!t.get_state().ShowTerminalStat().equals("OFF") && t.get_ongoingcommunication() == null){
							if (t2.get_state().ShowTerminalStat().equals("IDLE")){
								if (videoorvoice.equals("VOICE")) {
									Voice c = new Voice(_communicationid++, 0, t, t2, 0);
									c.set_communicationstate(true);
									t.set_ongoingcommunication(c);
									t.set_previousStat(t.get_state());
									t2.set_previousStat(t2.get_state());
									t.set_state(new Occupied());
									t2.set_state(new Occupied());

								} else if (videoorvoice.equals("VIDEO")) {
									if (t.ShowTerminalType().equals("Fancy")) {
										if (t2.ShowTerminalType().equals("Fancy")) {
											Video c = new Video(_communicationid++, 0, t, t2, 0);
											c.set_communicationstate(true);
											t.set_ongoingcommunication(c);
											t.set_previousStat(t.get_state());
											t2.set_previousStat(t2.get_state());
								...
								if (t2.get_state().ShowTerminalStat().equals("OFF")) {
									throw new DestinationisOffException(terminalKeyreceiver);
								}
								if (t2.get_state().ShowTerminalStat().equals("BUSY")) {
									throw new DestinationIsBusyException(terminalKeyreceiver);
								}
								if (t2.get_state().ShowTerminalStat().equals("SILENCE")) {
	...
	public void EndInteractiveCommunication(String terminalKeysender, double tempo) throws TerminalKeyInvalidException, UnknownTerminalException, CantStartCommunicationException {
		if (terminalKeysender.length() == 6 && terminalKeysender.matches("(\\d{6})")) {
			Terminal t = _terminals.get(terminalKeysender);
			if (t != null) {
				if (t.get_ongoingcommunication() != null) {
					t.get_ongoingcommunication().set_communicationstate(false);
					t.set_state(t.get_previousStat());
					t.set_previousStat(new Occupied());
					t.get_ongoingcommunication().get_terminalReceiver().set_state(t.get_ongoingcommunication().get_terminalReceiver().get_previousStat());
					t.get_ongoingcommunication().get_terminalReceiver().set_previousStat(new Occupied());
					t.updatePendingNotifications();
					t.sendpendingnotification();
					t.get_ongoingcommunication().get_terminalReceiver().updatePendingNotifications();
					t.get_ongoingcommunication().get_terminalReceiver().sendpendingnotification();
					...
					_communications.put(t.get_ongoingcommunication().get_id(), t.get_ongoingcommunication());
					...
					t.set_ongoingcommunication(null);
					t.get_owner().get_level().updateLevel();
				} else {
					throw new CantStartCommunicationException(terminalKeysender);
				}
