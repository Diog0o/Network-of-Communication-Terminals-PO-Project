...
public class Gold extends Level {
	...
	@Override
	public long cost(Communication communication) {
		if (communication.ShowTypeofCommunication().equals("TEXT")){
			...
			if (communication.get_terminalSender().AreFriends(communication.get_terminalReceiver(), communication.get_terminalSender())){
				return (long) ((communication.get_minutes() * 20)/2);
			}
			return (long) (communication.get_minutes() *20);
		}else{
			if(communication.get_terminalSender().AreFriends(communication.get_terminalReceiver(), communication.get_terminalSender())){
				return (long) ((communication.get_minutes() * 10)/2);
			}
			return (long) (communication.get_minutes() * 10);
		}
	}
}
